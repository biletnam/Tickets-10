...
if (count($today_items) > 0) {
  ...
  $pageParams['today'] = $r->txt->do_template($module, 'planner.today', $pageParams);
} else {
  $pageParams['today'] = $r->txt->do_template($module, 'planner.today.empty');
}
...
if (count($overdue_items) > 0) {
  ...
  $pageParams['overdue'] = $r->txt->do_template($module, 'planner.overdue', $pageParams);
} else {
  $pageParams['overdue'] = $r->txt->do_template($module, 'planner.overdue.empty');
} 
...
if (count($future_items) > 0) {
  ...
  $pageParams['future'] = $r->txt->do_template($module, 'planner.future', $pageParams);
} else {
  $pageParams['future'] = $r->txt->do_template($module, 'planner.future.empty');
}

$pageParams['pagetitle'] = $page->add('title',  $r->txt->do_template($module, 'title.planner', $pageParams);
$page->add('main', $r->txt->do_template($module, 'planner', $pageParams);
