...
if ($id > 0) {
  ...
  if ($op == 'set') {
    if ($price == 0) {
      $runtime->$_SESSION['flash'] = 'Please specify all the fields including the price');
    } elseif($price_currency == 0) {
      $runtime->$_SESSION['flash'] = 'Please specify all the fields including the currency');
    } elseif(count($periods) == 0) {
      $runtime->$_SESSION['flash'] = 'Please check at least one of the apartments/periods');
    } else {
      ...
      foreach $apt_period (@periods) {
        ($apttype, $period) = split('_', $apt_period, 2);
        $runtime->db->sqlrun($module, 'DeleteHotelApartmentPrice', array('apt_type' => $apttype, 'period' => $period));
      }
  }
  go("?p=resorts/edithotelprices&id=$id");
} else {
  $runtime->$_SESSION['flash'] = 'No hotel selected for setting the price');
  go("?p=resorts/hotels");
}
