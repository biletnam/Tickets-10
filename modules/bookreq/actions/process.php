...
#    formdebug($_REQUEST); print spreview($module, 'InsertFirstGuest', $_REQUEST);
    $cid1 = $_REQUEST['clientid'] = $runtime->sid($module, 'InsertFirstGuest', $_REQUEST); 
  }
  ...
  if ($cid1 > 0) {
    ...
    $bookid = $runtime->sid($module, 'InsertBooking', $_REQUEST);
    if ($bookid > 0) {
      # Add family members
      $sqlParams = array(
      ...
      $id1 = $runtime->sid($module, 'InsertFamilyGuest', $sqlParams) if ($_REQUEST['fname1'] . $_REQUEST['lname1'] <> '');
      $sqlParams = array(
      ...
      $id2 = $runtime->sid($module, 'InsertFamilyGuest', $sqlParams) if ($_REQUEST['fname2'] . $_REQUEST['lname2'] <> '');
      ...
      if ($_REQUEST['morepeople'] <> '') {
        ...
        foreach $fm (@morepeople) {
          ...
          $id2 = $runtime->sid($module, 'InsertFamilyGuest', $sqlParams) if ($fm['fname'] . $fm['lname'] <> '');
        }
      }
      ...
      $_SESSION['error'] = 'Could not create a booking');
    }
  } else {
    $_SESSION['error'] = 'Could not create a client for a booking');
  }
  go("?p=$module/view&id=$id");
} else {
  $_SESSION['error'] = 'Invalid booking request was requested to be converted');
  go("?p=$module/list");
}
