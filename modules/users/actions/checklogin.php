...
if ((scalar keys %userInfo) == 0) {
  ...
  if ((scalar keys %sessionInfo) > 0) {
    ...
    $runtime->db->sqlrun('users', 'UpdateUserSession', $sessionInfo); # TODO screen width and separators are not updated 
  } else {
    ...
    $runtime->db->sqlrun('users', 'InsertUserSession', $sessionInfo); 
  }
  set_cookie('sessionID', $sessionInfo['sessionID']);
  # register pageview
  $runtime->db->sqlrun('main', 'RegisterPageview', array('entity_type' => 'employee_login', 'entity_id' => $userInfo['id'], 'viewer_type' => 'U', 'viewer_id' => 0));    
