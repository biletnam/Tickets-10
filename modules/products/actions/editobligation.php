...
if ($product_id > 0) {
  ...
  if ($id > 0) {
    $page->add('title',  $obligationInfo['pagetitle'] = $r->txt->do_template($module, 'title.editobligation', $obligationInfo);
  } else {
    $obligationInfo['product_id'] = $product_id; 
    $page->add('title',  $obligationInfo['pagetitle'] = $r->txt->do_template($module, 'title.editobligation.new', $obligationInfo);
  } 
}

$page->add('main', $r->txt->do_template($module, 'editobligation', $obligationInfo);
