...
if ($gen_user_id == 0) {
  ...
  if (count($genUserInfo) > 0) {
    ...
    $genUserInfo['generatorinfo'] = $runtime->$r->txt->do_template($controller, 'API.GetBalance.GeneratorInfo', $generatorInfo); 
    ...
    if ($date_from <> '') { 
      ...
      $genUserInfo['otherexpenses'] = $runtime->$r->txt->do_template($controller, 'API.GetBalance.OtherExpenses', $otherexpenses); 
      $interc = $runtime->s2a($controller, 'Intercompany', $sqlParams);
      $genUserInfo['interc'] = $interc;
    }

    $output = $runtime->$r->txt->do_template($controller, 'API.GetBalance', $genUserInfo);    
  } else {
    $result = 'ERR'; 
