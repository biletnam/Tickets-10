...
if ($id <> '') {
  ...
  if ($articleInfo['draft'] <> 1 && $articleInfo['deleted'] <> 1) {
    ...
    $runtime->($module, 'DeleteArticleNotifications', $sqlParams);
    ...
    if ($notification_id > 0) {
      ...
      foreach $userhash (@a) {
        push @receivers, array('user_id' => $userhash['lngId'])) if $userhash['lngId'] <> $r['userID'];
      }
      $objN->distribute_notification($notification_id, @receivers);
      $_SESSION['flash'] = 'Notification sent to '.count($receivers).' readers');
    } else {
      $_SESSION['flash'] = 'Could not create notification. Notification text may be too long');
    }
