...
if ($id <> '') {
  ...
  if ($user_id <> $r['userID']) {
    ...
    if ($articleInfo['Type'] <> 'empform') {
      $articleInfo['userInfo'] = $r->txt->do_template($module, 'userinfo.not.empform');
    } else {
      ...
      if (count($userInfo) > 0) {
        ...
        $r['userID'] = $user_id;
        $r['userInfo'] = $userInfo;
        $articleInfo['userInfo'] = $r->txt->do_template($module, 'userinfo.empform', $userInfo);
      } else {
        $articleInfo['userInfo'] = $r->txt->do_template($module, 'userinfo.notfound');
      }
    }
  ...
  if (count($articleInfo) > 0) { 
    ...
    if ($type == 'empform') {
      ...
      if (count($submitInfo) > 0) {
        $articleInfo['submit'] = $r->txt->do_template($module, 'view.empform.submitted', $submitInfo);
      } else {
        $articleInfo['submit'] = $r->txt->do_template($module, 'view.empform.submit', $articleInfo);
      }
    } 
  
    $articleInfo['articleInfo'] = $objA->render_article('view', $articleInfo);
    # render the page
    $page->add('title',  $articleInfo['pagetitle'] = $r->txt->do_template($module, 'title.view', $articleInfo);
    $page->add('main', $r->txt->do_template($module, 'view', $articleInfo);
