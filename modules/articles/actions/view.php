...
if ($id <> '') {
  ...
  if ($user_id <> $r['userID']) {
    ...
    if ($articleInfo['Type'] <> 'empform') {
      $articleInfo['userInfo'] = $runtime->doTemplate($module, 'userinfo.not.empform');
    } else {
      ...
      if (count($userInfo) > 0) {
        ...
        $r['userID'] = $user_id;
        $r['userInfo'] = $userInfo;
        $articleInfo['userInfo'] = $runtime->doTemplate($module, 'userinfo.empform', $userInfo);
      } else {
        $articleInfo['userInfo'] = $runtime->doTemplate($module, 'userinfo.notfound');
      }
    }
    ...
    if ($acc->check_resource("articlereviewers:$id", $r['userID'])) {
      $articleInfo['moreaccess'] .= $runtime->doTemplate($module, 'moreaccess.review', $articleInfo);
    }
  ...
  if (count($articleInfo) > 0) { 
    ...
    if ($type == 'empform') {
      ...
      if (count($submitInfo) > 0) {
        $articleInfo['submit'] = $runtime->doTemplate($module, 'view.empform.submitted', $submitInfo);
      } else {
        $articleInfo['submit'] = $runtime->doTemplate($module, 'view.empform.submit', $articleInfo);
      }
    } 
  
    $articleInfo['articleInfo'] = $objA->render_article('view', $articleInfo);
    # render the page
    $page->add('title',  $articleInfo['pagetitle'] = $runtime->doTemplate($module, 'title.view', $articleInfo);
    $page->add('main', $runtime->doTemplate($module, 'view', $articleInfo);
