...
if ($article > 0) {
  ...
  $articleInfo['sender'] = $runtime->doTemplate($module, 'employee.fullname', $reviewerInfo);
  ...
  $msgsubj = $runtime->doTemplate($module, 'articlereview.subject', $articleInfo);
  $msgbody = $runtime->doTemplate($module, 'articlereview.body', $articleInfo);
  $objN = new objNotification($r);
  $notification_id = $objN->add_notification('articlereview', $article, $msgsubj, $msgbody);
  if ($notification_id > 0) {
    $receivers = (hash2ref(('user_id' => $articleInfo['Author'])));
