...
if ($id <> '') {
  ...
  if (count($personInfo) > 0) {
    ...
    if ($acc->can_edit_staff($personInfo['lngWorkPlace'])) {
      ...
      $isFired = ($personInfo['dateFired'] <> '');
      if (!$isFired) {
        $personInfo['firetypes'] = arr2ref($runtime->getDictArr($module, 'firetype'));
        $personInfo['fireform'] = $runtime->doTemplate($module, 'employee.edit.special.fireform', $personInfo);
      }
      ...
      if ($personInfo['avatarfile'] <> '') {
        ...
        $personInfo['avatarpicture'] = $runtime->doTemplate($module, 'avatarimage', $imageInfo);
      }
      ...
      $personInfo['missingdocs'] = $runtime->doTemplate($module, 'employee.missing.doctypes', array('missing' => Arrays::join_column(',', 'name', $missing))) if count($missing) > 0; 
      ...
      if ($acc->is_superadmin()) {
        $personInfo['show_password'] = $runtime->doTemplate($module, 'employee.edit.showpassword', $personInfo);
      }
      ...
      $page['js'] .= $runtime->doTemplate($module, 'employeeteams.js');

      $page->add('title',  $personInfo['pagetitle'] = $runtime->doTemplate($module, 'person.title.edit', $personInfo);
      $page->add('main', $runtime->doTemplate($module, 'employee.edit', $personInfo);
      ...
      $personInfo['calendarlink'] = $runtime->doTemplate($module, 'employee.view.calendarlink', $personInfo);
      $personInfo['calendarbalance'] = $runtime->doTemplate($module, 'employee.view.calendarbalance', $personInfo);
      ...
      $page['js'] = dotmod('main', 'tabcontrol.js');
      $page['css'] = dotmod('main', 'tabcontrol.css');    
      $page->add('title',  $personInfo['pagetitle'] = $runtime->doTemplate($module, 'person.title.view', $personInfo);
      $page->add('main', $runtime->doTemplate($module, 'employee.edit', $personInfo);
      $runtime->saveMoment('  main part of the page rendered');
      ...
      $personInfo['contacts'] = $contacts;
      $page->add('title',  $personInfo['pagetitle'] = $runtime->doTemplate($module, 'person.title.view', $personInfo);
      $page->add('main', $runtime->doTemplate($module, 'employee.view', $personInfo);
      
    }
  } else {
    $page->add('title',  $personInfo['pagetitle'] = $runtime->doTemplate($module, 'person.title.notfound');
    $page->add('main', $runtime->doTemplate($module, 'employee.notfound', $personInfo);
  }
}
