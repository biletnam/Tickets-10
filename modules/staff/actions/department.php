...
if ($id <> 0) {
  ...
  $page->add('title',  $departmentInfo['pagetitle'] = $r->txt->do_template($module, 'title.department', $departmentInfo);
  if ($acc->can_edit_department($id)) {
    $departmentInfo['editform'] = $r->txt->do_template($module, 'department.editform', $departmentInfo);
    ...
    $departmentInfo['tabcontrol'] = $r->txt->do_template($module, 'department.details', $departmentInfo) . dot('department.staff', $departmentInfo);
  }
  $page['js'] .= $r->txt->do_template($module, 'search.sort.js');
  $page->add('main', $r->txt->do_template($module, 'department', $departmentInfo);
} else {
  $page->add('main', $r->txt->do_template($module, 'department.notfound', $departmentInfo);
}
