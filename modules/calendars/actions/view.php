...
if ($special == 'employeeabsence') {
  ...
  if (count($specialInfo) == 0) {
    $page->add('main', $r->txt->do_template($module, 'view.create.absence.calendar', array('id' => $employee_id));
  } else {
    $id = $specialInfo['id'];
    # register pageview
...
#    print '  access checked with result: '.$access;
    ...
    if ($access == 'none') {
      $page->add('main', $r->txt->do_template($module, 'view.noaccess');
    } else {
...
#      $tcCalendar->addTab('week', dot('view.tab.week', $articleInfo), dot('view.wait.week', $calendarInfo));
      ...
      # render the page
      $page->add('title',  $calendarInfo['pagetitle'] = $r->txt->do_template($module, 'title.view', $calendarInfo);
      $page->add('main', $r->txt->do_template($module, 'view', $calendarInfo);
    }
  } else {
    $page->add('title',  $calendarInfo['pagetitle'] = $r->txt->do_template($module, 'title.view.notfound');
    $page->add('main', $r->txt->do_template($module, 'view.notfound', $calendarInfo);
  }
  
  $page['js'] .= $r->txt->do_template('main', 'tabcontrol.js');
  $page['js'] .= $r->txt->do_template('main', 'linkpeople.js');
  $page['js'] .= $r->txt->do_template($module, 'calendar.js', array('id' => $id));
