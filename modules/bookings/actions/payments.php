...
if (count($invoices) == count($invoiceissues)) {
  $pageParams['newissue'] = $runtime->doTemplate($module, 'payments.allissued');
} else {
 ...
 # print Dumper($invoices2issue);
  ...
  if (count($newissue_currencyoptions) == 1) {
    $pageParams['currency'] = $runtime->doTemplate($module, 'payments.newissue.currency.fixed', ${$newissue_currencyoptions[0]});
  } else {
    $pageParams['currency'] = $runtime->doTemplate($module, 'payments.newissue.currency.select', array('currencyoptions' => $newissue_currencyoptions));
  }
...
#    print Dumper($newissue_payeroptions[0]);
    $pageParams['payer'] = $runtime->doTemplate($module, 'payments.newissue.payer.fixed', ${$newissue_payeroptions[0]});
    $pageParams['invoices2issue'] = arr2ref(genCheckboxes($invoices2issue, 'invoice_ids', 'id', 'comment'));
  } else {
    $pageParams['payer'] = $runtime->doTemplate($module, 'payments.newissue.payer.select', array('book_id' => $pageParams['book_id'], 'payeroptions' => $newissue_payeroptions));
  }
  
  
  # TODO - on practice, Yury restricts all the lists - see boking_invoice.pm, function NewIssue
  $pageParams['newissue'] = $runtime->doTemplate($module, 'payments.newissue', $pageParams);
}

if (count($invoiceissues) > 0) {
 # print 'invoice issues ' . Dumper($invoiceissues);
  $pageParams['invoiceissues'] = $runtime->doTemplate($module, 'payments.invoiceissues', array('invoiceissues' => $invoiceissues)); 
}


$orphanexpenses = $runtime->s2a($module, 'ListOrphanExpenses', $pageParams);
if (count($orphanexpenses) > 0) {
  $pageParams['orphanexpenses'] = $runtime->doTemplate($module, 'payments.orphanexpenses', array('orphanexpenses' => $orphanexpenses));
}
