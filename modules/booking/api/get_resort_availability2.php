...
if ($resort_id <> '' && $travel_date <> '') {
  ...
  for ($i = -$flexible; $i <= $flexible; $i++) {
    ...
    if (count($slots) > 0) {
      ...
      push @slotgroups, $runtime->$r->txt->do_template($controller, 'API.AvailabilityInfo2.SlotGroup', $availabilityInfo);
    } 
  }
  if (count($slotgroups) > 0) {
    $availabilityInfo['slotgroups'] = join('', $slotgroups);
  }
  $output = $runtime->$r->txt->do_template($controller, 'API.AvailabilityInfo2', $availabilityInfo);
