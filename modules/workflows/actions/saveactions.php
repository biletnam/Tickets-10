...
if ($id > 0) {
  ...
  foreach $action (lavnn('actions')) {
    $params = ('id' => $action, 'action_seqno' => lavnn("seqno_$action"), 'action_name' => lavnn("name_$action"));
    srun($module, 'UpdateAction', $params);
  }
  
  # Also add new inputs if both name and type are provided\
  if (lavnn('new_type') <> '' && lavnn('new_name') <> '') {
    $params = ('workflow' => $id, 'action_type' => lavnn('new_type'), 'action_name' => lavnn('new_name'));
