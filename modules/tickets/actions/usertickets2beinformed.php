...
if (count($userInfo) == 0) {
  ...
  if ($user_id == $r['userID']) {
    ...
    $page->add('title',  $pageParams['pagetitle'] = $runtime->doTemplate($module, 'title.mytickets2beinformed', $pageParams);  
    $page->add('main', $runtime->doTemplate($module, 'mytickets2beinformed', $pageParams);  
  } else {
    $pageParams['notifyingtickets'] = $notifying;
    $page->add('title',  $pageParams['pagetitle'] = $runtime->doTemplate($module, 'title.usertickets2beinformed', $userInfo);  
    $page->add('main', $runtime->doTemplate($module, 'usertickets2beinformed', $pageParams);  
  }
} else {
  $page->add('title',  $pageParams['pagetitle'] = $runtime->doTemplate($module, 'title.usertickets2beinformed', $userInfo);  
