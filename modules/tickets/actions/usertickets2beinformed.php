...
if (count($userInfo) == 0) {
  ...
  if ($user_id == $r['userID']) {
    ...
    $page->add('title',  $pageParams['pagetitle'] = $runtime->txt->do_template($module, 'title.mytickets2beinformed', $pageParams);  
    $page->add('main', $runtime->txt->do_template($module, 'mytickets2beinformed', $pageParams);  
  } else {
    $pageParams['notifyingtickets'] = $notifying;
    $page->add('title',  $pageParams['pagetitle'] = $runtime->txt->do_template($module, 'title.usertickets2beinformed', $userInfo);  
    $page->add('main', $runtime->txt->do_template($module, 'usertickets2beinformed', $pageParams);  
  }
} else {
  $page->add('title',  $pageParams['pagetitle'] = $runtime->txt->do_template($module, 'title.usertickets2beinformed', $userInfo);  
