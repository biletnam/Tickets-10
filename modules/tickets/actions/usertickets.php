...
if (count($userInfo) == 0) { 
  ...
  if ($pageParams['user_id'] == $r['userID']) { 
    ...
    $page->add('main', $runtime->txt->do_template($module, 'mytickets', $pageParams);  
  } else { 
    $userInfo['stats'] = $stats;
    $page->add('title',  $userInfo['pagetitle'] = $runtime->txt->do_template($module, 'title.usertickets', $userInfo);
    $page->add('main', $runtime->txt->do_template($module, 'usertickets', $userInfo);  
  }
} else { 
