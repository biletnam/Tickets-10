...
$lastweek_byday = Arrays::slice_array($lastweek, 'ReportingDate'); $lastweek = array();
$thisweek_byday = Arrays::slice_array($thisweek, 'ReportingDate'); $thisweek = array();
...
foreach $date (sort (keys(%thisweek_byday))) {
  ...
  $dayreport['details'] = $runtime->doTemplate($module, 'usertime.dayreport.list', array('reports' => $dayreports));
...
$time_by_months = '';
if (count($bymonth) > 0) {
  $byyear = Arrays::slice_array($bymonth, 'Y'); 
  foreach $Y (reverse sort keys %byyear) {
    $time_by_months .= $runtime->doTemplate($module, 'usertime.year', array('year' => $Y, 'months' => loopt('usertime.year.month', @{$byyear{$Y}})));
  }
  $time_by_months = $runtime->doTemplate($module, 'usertime.bymonths', array('selector' => $time_by_months));
...
if ($r['userID'] == $user_id) {
  $page->add('title',  $pageParams['pagetitle'] = $runtime->doTemplate($module, 'title.mytime', $pageParams);  
} else {
  $page->add('title',  $pageParams['pagetitle'] = $runtime->doTemplate($module, 'title.usertime', $userInfo);  
}
$page->add('main', $runtime->doTemplate($module, 'usertime', $pageParams);
