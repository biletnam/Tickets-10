...
if (count($userInfo) == 0) {
  ...
  if ($user_id == $r['userID']) {
    $page->add('title',  $pageParams['pagetitle'] = $runtime->txt->do_template($module, 'title.mytickets2handle', $pageParams);  
    $page->add('main', $runtime->txt->do_template($module, 'mytickets2handle', $pageParams);  
  } else {
    # TODO check if this user is somehow related to this person (HR? LM? DS?) and restrict access
    $page->add('title',  $pageParams['pagetitle'] = $runtime->txt->do_template($module, 'title.usertickets2handle', $userInfo);  
    $page->add('main', $runtime->txt->do_template($module, 'usertickets2handle', $pageParams);  
  }
} else {
  $page->add('title',  $pageParams['pagetitle'] = $runtime->txt->do_template($module, 'title.usertickets2handle', $userInfo);  
