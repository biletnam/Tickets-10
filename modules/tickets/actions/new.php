...
if ($target <> '') {
  ...
  foreach (split(',', $target) as $t) {
    ...
    if (count($moretickets) > 0) {
      ...
      $ti['moreticketslink'] = $r->txt->do_template($module, 'new.target.more', $ticketInfo);
    }
    $targets[] = $ti;
  }
...
$ticketInfo['pagetitle'] = $r->txt->do_template($module, 'title.new');
$page->add('title', $ticketInfo['pagetitle']);
$page->add('main', $r->txt->do_template($module, 'new', $ticketInfo));
