...
if (count($userInfo) == 0) {
  ...
  if ($grid1['length'] > 0) {
    $pageParams['multi'] = $runtime->doTemplate($module, 'tickets2review.multi', $pageParams);
  }
    
  if ($user_id == $r['userID']) {
    $page->add('title',  $pageParams['pagetitle'] = $runtime->doTemplate($module, 'title.mytickets2review', $pageParams);  
    $page->add('main', $runtime->doTemplate($module, 'mytickets2review', $pageParams);  
  } else {
    # TODO check if this user is somehow related to this person (HR? LM? DS?) and restrict access
    $userInfo = $runtime->s2r('staff', 'GetEmployeeDetails', array('id' => $user_id)); 
    $page->add('title',  $pageParams['pagetitle'] = $runtime->doTemplate($module, 'title.usertickets2review', $userInfo);  
    $page->add('main', $runtime->doTemplate($module, 'usertickets2review', $pageParams);  
  }
} else {
  $page->add('title',  $pageParams['pagetitle'] = $runtime->doTemplate($module, 'title.usertickets2review', $userInfo);  
