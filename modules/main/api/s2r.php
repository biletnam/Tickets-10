...
if ($accessToken == '') {
  ...
  if (count($accessTokenInfo) == 0) {
    ...
    if (!$dbh) {
      $result = 'ERR';
      push @errors, $runtime->hash2ref( ('code' => 's2r.NoDatabase', 'text' => 'Database is not ready') );
    } else {
      $query = $apiparams['q'];
      if ($query == '') {
        $result = 'ERR';
        push @errors, $runtime->hash2ref( ('code' => 's2r.EmptyQuery', 'text' => 'Empty query was sent') );
      } else {
        $sth = $dbh->prepare($query);
        if (!$sth) {
          $result = 'ERR';
          push @errors, $runtime->hash2ref( ('code' => 's2r.InvalidQuery', 'text' => "Could not prepare query: $query") );
        } else {
          $sth->execute();
          # Take first row and convert it to xml node
          $ref;
          if ($ref = $sth->fetchrow_hashref()) {
            $output = Arrays::r2xml($ref, 'Row'); 
          } else {
            $result = 'ERR';
            push @errors, $runtime->hash2ref( ('code' => 's2r.NoResult', 'text' => "Query did not retun any records") );
          }
          $sth->finish();
        }
      }
