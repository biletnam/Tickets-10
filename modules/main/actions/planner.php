...
if (count($today_items) > 0) {
  ...
  $pageParams['today'] = $runtime->doTemplate($module, 'dashboard.planner.today', $pageParams);
} else {
  $pageParams['today'] = $runtime->doTemplate($module, 'dashboard.planner.today.empty');
}
...
if (count($overdue_items) > 0) {
  ...
  $pageParams['overdue'] = $runtime->doTemplate($module, 'dashboard.planner.overdue', $pageParams);
} 
...
if (count($future_items) > 0) {
  ...
  $pageParams['future'] = $runtime->doTemplate($module, 'dashboard.planner.future', $pageParams);
} else {
  $pageParams['future'] = $runtime->doTemplate($module, 'dashboard.planner.future.empty');
}
