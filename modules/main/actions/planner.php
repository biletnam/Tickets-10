...
if (count($today_items) > 0) {
  ...
  $pageParams['today'] = $r->txt->do_template($module, 'dashboard.planner.today', $pageParams);
} else {
  $pageParams['today'] = $r->txt->do_template($module, 'dashboard.planner.today.empty');
}
...
if (count($overdue_items) > 0) {
  ...
  $pageParams['overdue'] = $r->txt->do_template($module, 'dashboard.planner.overdue', $pageParams);
} 
...
if (count($future_items) > 0) {
  ...
  $pageParams['future'] = $r->txt->do_template($module, 'dashboard.planner.future', $pageParams);
} else {
  $pageParams['future'] = $r->txt->do_template($module, 'dashboard.planner.future.empty');
}

print $r->txt->do_template($module, 'dashboard.planner', $pageParams);
?>
